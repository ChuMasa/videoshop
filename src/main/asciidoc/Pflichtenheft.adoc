...
Das Glossar enthält eine Liste aller in diesem Projekt verwendeten Wörter und Ausdrücke, für die eine Beschreibung erforderlich ist, um Missverständnisse zwischen den Beteiligten zu vermeiden.
Weitere Definitionen von Begriffen finden Sie auch in der Liste der Akteure, Akteure, Stakeholder und im Domänenmodell.

_Hinweis: Einige Begriffe können während eines Projekts regelmäßig verwendet werden, während alle Beteiligten der Ansicht sind, dass die Bedeutung offensichtlich ist. Dies ist jedoch nicht unbedingt der Fall, da unterschiedliche Fachgebiete unterschiedliche Wissensstände oder einfach ein unterschiedliches Verständnis eines Begriffs bedeuten können
_Ein Beispiel aus einem früheren Jahr dieses Kurses: _ +
_Stellen Sie sich einen Schichtplan vor, in dem jede Schicht mit 3 verschiedenen Mitarbeitern besetzt ist._
_Der für den Zeitplan verantwortliche Manager würde den Begriff "Schicht" verwenden, um die gesamte Zeitspanne mit allen drei beteiligten Mitarbeitern zu beschreiben (z. B. "Schicht X wird für euch schwer, bereitet euch vor") ._
_Ein Mitarbeiter, der einen dieser Zeitnischen belegt, würde den Begriff "Schicht" verwenden, um seinen einen Zeitnischen (von den dreien) in einem Zeitschlitz des Tages zu beschreiben (z. B. "Meine Schicht bringt mich diesmal mit den Kunden in Kontakt, während die die beiden anderen können sich im lager entspannen ") ._ +
_Während dies gesunder Menschenverstand ist und die Kommunikation in der realen Welt nicht wirklich beeinträchtigt, wird es zu einem Problem, wenn Sie ein System entwerfen müssen, das einen solchen Schichtplan darstellt. Sie könnten in diesem Fall "shift" wie im Verständnis des Managers verwenden und "slot" oder "cell" verwenden, um die Bedeutung des Mitarbeiters zu modellieren.
_In solchen Fällen müssen Sie alle Beteiligten zwingen, diesen gemeinsamen Wortlaut zu verwenden, um Missverständnisse zu vermeiden ._
